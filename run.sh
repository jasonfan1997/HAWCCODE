...
aerie-apps-scrappy-cut-dataset --cuts /data/disk01/home/jasonfan/crab/ocut --outfileFormat $2$outfile < $2$outname
rm $2$outname
#for i in `find $test -name "Afterocut_*_$inputfilename" -type f`; do
for i in {0..119}
do 
	dir=$2$i
	mv $2$ppre$i$und$inputfilename $dir
done
for i in {0..119}
do 
	dir=$test$i
	mv $test$ppre$i$und$inputfilename $dir
done	