...
int main(int argc, char** argv){
  ...
  cl.AddPositionalOption<vector<string> >("input", "HAWC rec XCDF file(s) or port address (online only)");
  ...
  vector<string> recfile = cl.GetArgument<vector<string> >("input");
  if(recfile.size() != 1){
    cout<<"ONLY ONE INFILE ATM"<<endl;
    exit(1);
  }
